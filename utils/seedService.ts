  ...
  try {
    ...
    if (existingItems.length === 0) {
      ...
      try {
        ...
        console.warn('Could not seed Firestore (might be blocked), using mock data:', seedError);
      }
    } else {
      console.log(`Found ${existingItems.length} existing food items`);
    }
  } catch (error) {
    console.warn('Error initializing Firestore data, will use mock data fallback:', error);
